...
const NavbarBeebambini = () => {
  ...
  const [activeBoysMenu, setActiveBoysMenu] = useState(false);
  const [activeGirlsMenu, setActiveGirlsMenu] = useState(false);
  ...
  const handleMenuToggle = (type) => {
    if (type === "collection") {
      ...
      setActiveGirlsMenu(false);
      setActiveBoysMenu(false);
      setActiveSubmenu(null);
    } else if (type === "boys") {
      setActiveBoysMenu(!activeBoysMenu);
      setActiveCollectionMenu(false);
      setActiveGirlsMenu(false);
      setActiveSubmenu(null);
    } else {
      setActiveGirlsMenu(!activeGirlsMenu);
      setActiveCollectionMenu(false);
      setActiveBoysMenu(false);
      setActiveSubmenu(null);
    }
  ...
  const handleClickOutside = (event) => {
    if (navRef.current && !navRef.current.contains(event.target)) {
      setActiveBoysMenu(null);
      setActiveGirlsMenu(null);
        ...
        <div className="w-full flex items-center justify-between px-2 lg:px-0 sticky top-10 z-10 bg-navBgColor">
          ...
          <ul className="gap-10 space-x-4 hidden md:flex" ref={navRef}>
            <NavBarDropDownMenu
              ...
              activeMenu={activeGirlsMenu}
              onToggleMenu={() => handleMenuToggle("girls")}
            ...
            <NavBarDropDownMenu
              ...
              activeMenu={activeBoysMenu}
              onToggleMenu={() => handleMenuToggle("boys")}
              onSubmenuToggle={handleSubmenuToggle}
              activeSubmenu={activeSubmenu}
              handleRedirect={handleRedirect}
            />
...
export default NavbarBeebambini;
